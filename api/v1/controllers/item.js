...
module.exports = {
  getAllMenuItems: async (req, res, _next) => {
    try {
      ...
      if (!items) {
        return res
          .status(404)
          .json({ errors: [{ msg: 'No menu items, please add some!' }] });
      }
  ...
  getCategoryItems: async (req, res, _next) => {
    try {
      ...
      const category_items = await Item.find({
        item_category: req.item.category,
      });
      if (!category_items) {
        return res
          .status(404)
          .json({ errors: [{ msg: 'No menu items for this cateogry exist' }] });
      }
      ...
      return res.status(500).json({
        errors: [
          {
            msg:
              'Unexpected server error happened. Could not get menu items for specified category.',
          },
        ],
      });
    }
    ...
    try {
        ...
        if (!user.admin) {
          return res.status(401).json({
            errors: [
              {
                msg:
                  'Unauthorized access, user is not an admin. Only admin can create an item!',
              },
            ],
          });
        }
        ...
        if (!itemFound) {
          return res.status(404).json({
            errors: [
              {
                msg:
                  'Menu item not found! Could not update the existing menu item',
              },
            ],
          });
        }
          ...
          if (sameItem) {
            ...
            return res.status(400).json({
              errors: [
                {
                  msg:
                    'Menu item name already exists. Please provide new name or entirely new menu item!',
                },
              ],
            });
          }
          ...
          if (samePicture) {
            return res.status(400).json({
              errors: [
                {
                  msg:
                    'Menu item picture already exists. Please provide new url to picture!',
                },
              ],
            });
          }
      ...
      return res.status(500).json({
        errors: [
          {
            msg:
              'Unexpected server error happened. Could not create a new menu item.',
          },
        ],
      });
    }
  ...
  updateAvailability: async (req, res, _next) => {
    ...
    // Destructuring data from request body.
    const { item_availability } = req.body;
    ...
    try {
      ...
      if (!user.admin && !user.staff) {
        return res.status(401).json({
          errors: [
            {
              msg:
                'Unauthorized access, user is not an admin or staff. Only admin or staff can update an item!',
            },
          ],
        });
      }
      ...
      return res.status(500).json({
        errors: [
          {
            msg:
              'Unexpected server error happened. Could not update availability of menu item.',
          },
        ],
      });
    }
